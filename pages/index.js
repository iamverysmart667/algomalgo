  ...
  return (
    <div className="flex items-center space-x-8">
      ...
      <div className={`bg-gradient-to-b from-gray-300 to-gray-300 bg-no-repeat bg-center bg-[length:1px] pb-8`}>
        {checked
          ? <div className={`${topic && 'mt-8'}`} onClick={() => setChecked(!checked)}><Check/></div>
          : <div className={`${topic && 'mt-8'} rounded-3xl w-6 h-6 bg-sky-50 border border-gray-400`} onClick={() => setChecked(!checked)}></div>
        }
      </div>
